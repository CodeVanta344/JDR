...
export function getStarterDeck(playerClass: string): Card[] {
  const frappes = Array(5).fill(null).map((_, i) => ({ ...CARD_FRAPPE, id: `frappe_${i}` }));
  const defenses = Array(4).fill(null).map((_, i) => ({ ...CARD_DEFENSE, id: `defense_${i}` }));

  const cls = (playerClass || '').toLowerCase();

  if (cls.includes('guerrier') || cls.includes('warrior') || cls.includes('paladin')) {
    return [...frappes, ...defenses, { ...CARD_COUP_PUISSANT, id: 'coup_puissant_0' }];
  }
  if (cls.includes('mage') || cls.includes('sorcier') || cls.includes('wizard')) {
    const defs3 = defenses.slice(0, 3);
    return [...frappes, ...defs3, { ...CARD_ECLAIR, id: 'eclair_0' }, { ...CARD_BOUCLIER_ARCANE, id: 'bouclier_0' }];
  }
  if (cls.includes('rôdeur') || cls.includes('rodeur') || cls.includes('ranger') || cls.includes('archer')) {
    const defs3 = defenses.slice(0, 3);
    return [...frappes, ...defs3, { ...CARD_TIR_PRECIS, id: 'tir_precis_0' }, { ...CARD_ESQUIVE, id: 'esquive_0' }];
  }
  if (cls.includes('clerc') || cls.includes('cleric') || cls.includes('prêtre')) {
    const defs3 = defenses.slice(0, 3);
    return [...frappes, ...defs3, { ...CARD_LUMIERE, id: 'lumiere_0' }, { ...CARD_DEFENSE, id: 'defense_extra' }];
  }
